...
### Bulk recipe organization

The [`POST /api/recipes/bulk-actions/organize` endpoint](https://demo.mealie.io/docs#/Recipe%3A%20Bulk%20Actions/bulk_organize_recipes) adds or removes tags and categories from multiple recipes in one atomic operation. It uses the same permissions and group/household boundaries as individual recipe edits.
...
`recipes` must contain at least one unique recipe UUID. `operation` is either `add` or `remove`. `tags` and `categories` are independently optional; send an empty list for one organizer type when it should remain unchanged. Organizer objects must identify existing tags or categories in the caller's group. Duplicate organizer entries are reduced to one entry.

The operation is idempotent. Adding an organizer that a recipe already has, or removing one that it does not have, does not change that recipe. The response is HTTP 200 and contains the changed recipes as `RecipeSummary` objects. If no recipe changed, the response is an empty list:
...
Validation and authorization happen before changes are committed, so a failed request does not partially update the selected recipes:

| Status | Meaning |
| --- | --- |
| `200` | The request succeeded; the body contains changed recipe summaries, or `[]` when nothing changed. |
| `403` | The caller cannot edit every selected recipe. |
| `404` | A selected recipe, tag, or category is not available in the caller's group. |
| `422` | The request body is invalid, including an empty or duplicate recipe selection. |

The generated [OpenAPI/Swagger reference](https://demo.mealie.io/docs) remains the canonical reference for the full request and `RecipeSummary` schemas.
